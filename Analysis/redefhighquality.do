...
foreach file in county countyCON {
/******************************************************************************/
use temp_`file'_P.dta, clear /*LOOPS OVER BOTH FILES!*/
...
foreach service in AR FR MR NR{
 ...
 areg LN`service'monthcounty monthcountydeath L1monthcountydeath outofcounty L1outofcounty stateunemp countyunemp monthfe3-monthfe58 statetrend2-statetrend51 [aweight=avgcountypop], robust cluster(fips) absorb(fips)
...
foreach TYPE in LQ HQ50 HQ50alt HQ75 {
 gen LN`TYPE'monthcounty=ln(R`TYPE'monthcounty)
 areg LN`TYPE'monthcounty monthcountydeath L1monthcountydeath outofcounty L1outofcounty  stateunemp countyunemp monthfe3-monthfe58 statetrend2-statetrend51 [aweight=avgcountypop], robust cluster(fips) absorb(fips)
 outreg2 monthcountydeath L1monthcountydeath outofcounty L1outofcounty stateunemp countyunemp using ./Output/highqualitybytypeLN.txt, ct(`file'`TYPE') bdec(3) tdec(3) bracket se append
}
*/
/*RECRUITS OF DIFFERENT QUALITY--POISSON*/
foreach TYPE in LQ HQ50 HQ50alt HQ75 {
 xtpoisson R`TYPE'monthcounty monthcountydeath L1monthcountydeath outofcounty L1outofcounty stateunemp countyunemp monthfe3-monthfe58 statetrend2-statetrend51, fe exposure(countypop) vce(robust)
 outreg2 monthcountydeath L1monthcountydeath outofcounty L1outofcounty stateunemp countyunemp  using ./Output/highqualitybytypeP.txt, ct(`file'`TYPE') bdec(3) tdec(3) bracket se append 
