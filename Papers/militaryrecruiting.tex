...
\footnote{See \url{http://www.wbng.com/home/Serving-the-community-A-family-legacy-230269421.html}} The ``sandhogs'' who do the dangerous work of boring the tunnels under New York City often follow in (fallen) friends or family members footsteps.\footnote{See \url{http://www.villagevoice.com/2012-04-11/news/sandhogs-tunneling-second-avenue-subway/full/} or \url{http://www.nydailynews.com/archives/news/brave-sandhogs-pause-reflect-article-1.569895}} This possible incentive effect is clearly not unique to a single instance, and not unique to the military, though at least one example from the military received media attention: 61-year-old orthopedic surgeon Bill Krissoff acquired an age waiver and enlisted in the Navy Medical Corps after his son Nathan was killed in the Marines in Iraq.
...
Enduring Freedom (i.e., the war in Afghanistan) during the same period. With detailed geographic and date information, I am able to analyze recruiting at the county-month
...
Using Poisson maximum likelihood estimates of semi-elasticities, I
...
using least squares weighted by county population, and the estimates are very stable across different sets of fixed effects. I find that the effect is very localized---deaths
...
to military personnel and family.\footnote{\url{http://www.defenselink.mil/news/2010\%20Budget\%20Proposal.pdf}} As of July 2010, this money was being used to pay and support over
...
recruiters. Being a recruiter is very similar to other military occupational
...
used the plausibly exogenous variation in the geography of US deaths in Iraq at the state level
to examine the effect on changes in voteshare for George W. Bush between
...
\footnote{Compare http://www.nytimes.com/2001/11/12/us/nation-challenged-recruit-self-described-slacker-decides-he-s-ready-be-soldier.html,
which describes an individual motivated to enlist to http://www.nytimes.com/2001/09/16/us/after-attacks-military-despite-national-rush-emotion-recruiting-centers-aren-t.html,
...
for military population levels---the data is not tainted by service-men
...
Table \ref{Flo:Simple Deaths} shows total annual military deaths and the subsets of those recorded as hostile action from the FOIA data and those considered part of the Iraqi/Enduring Freedom operations as reported publicly by the DMDC. One can see that at most in any year,
under 50\% of the deaths of active duty US military members during the relevant time period (2001-2006) are classified as part of the Iraq/Afghanistan wars. Also, the Iraq/Afghanistan deaths outnumber the hostile deaths, so friendly fire or accidental deaths can be considered part of the war data. During the entire period for which I have data, 1999, two years prior to the beginning of my analysis, experienced the lowest number of deaths, with  796, of which zero were classified as hostile. 
...
\subsection{County Origin and Rates of Death}
...
Since I lack data on military occupational specialties, I can compare the number of
recruits from a state to the number of deaths from the same state.
Figure \ref{Flo:HISTO total deaths total recruits} shows histograms
of the ratio of active duty deaths to total active duty applicants
for each state over the whole period for which I have data. The ratios
are centered around 0.3\%, but are clearly not all identical. I have
repeated this exercise including both active duty and reserve and
guard deaths (since service and death in the reserve and guard duty
...
relatively small, ranging from 0.143 to 0.319. Simply put, recruits
have about the same likelihood of dying, regardless of where they
are from. 

Another way to analyze this is to look at each individual observation,
and check the likelihood that it came from a binomial distribution
with the hazard rate equal to that of the overall national hazard
rate. (The number of active-duty deaths divided by the total number
of active duty applicants was .003.) I then tested the likelihood
that each observation came from a binomial distribution with this
hazard rate of p=.003. Figure \ref{Flo:Pat's Histo} displays two
histograms of the p-value for each state, one using active-duty deaths
...
but to interpret, one should use an adjustment for multiple testing such as Bonferroni or Sidak corrected p-values
(i.e., divide the cutoff for significance by the number of tests,
51, thus replacing a cutoff of .05 with 0.05/51=0.0009). Only two
...
data, and only one, Massachusetts, rejects using contracts data. So
it is possible that recruits from certain states are more likely to
...
only be rejected for one or two states. Repeating this analysis with the 3,100 counties shows that rates of death given enlistment by county are also very rarely significantly different.

%Add test of serial correlation, or thereabouts.
...
this underlying characteristic of the state, however, it does give
a slightly different meaning to the estimates I will develop in the
next few pages. If deaths in Iraq and Afghanistan were truly uniformly
...
from different states have only slightly different hazard rates, and
...
The primary analysis is at the county level, the smallest region at which there is close to a one-to-one relationship from death data geographic unit to recruit data geographic unit. 

Since data on recruits is in count form, the Poisson model is a more natural one to fit to the data, and provides the results on which I place primary emphasis.\footnote{Linear models of course ignore the restriction of the dependent variable of recruits to non-negative integers. See \cite{cameron2013countdata} for a complete discussion. For the sake of robustness and transparency, I have also tested linear and log-linear specifications, which are shown in the Appendix. The results are qualitavely very similar, showing that a death leads to slightly larger decreases in percentage terms when estimated with log-linear OLS regressions than in the Poisson regressions. The appendix also includes a square root specification, as well as negative binomial. Again, the results are very similar.} Poisson regression fits a generalized linear model of the form $log(\mu_{i})=x'_{i}\beta$, so $\mu_{i}=exp(x'_{i}\beta)$
and a one unit increase in $x_{j}$ multiplies $\mu_{j}$ by $exp(\beta_{j})$.
However, as I am modeling an underlying rate of enlistment, $\mu_{i}=e^{x'_{i}\beta}$, the observed number of recruits is the rate times the exposure, which in my case is the population of young males. If $R_{i}$ is the expected number of recruits, then $R_{i}=Population_{i}\cdot e^{x'_{i}\beta}=e^{ln(Population_{i})+x'_{i}\beta}$.
...
Table \ref{Flo:Poisson Basic}
shows the results when Poisson regression is used to analyze the data
...
Fixed effects for each county and for each month are included for the remaining specifications, and state specific linear time trends in specifications 4 and 8.
...
in-county death is followed in the next month by a 0.79\% to 0.90\%
decrease in applicants and a 0.77\% to 0.84\% reduction in contracts.
...
assessment of the risk of death, but still, the magnitude of the observed
...
remain very similar. These results are shown in Appendix Table~\ref{Flo:Rdeaths}. 
...
place, I find it highly unlikely that the military is relocating them
in a way that is correlated with monthly deaths. Without being relocated,
however, recruiters may change their level of effort. FOIA requests for data on recruiter quotas unfortunately have not been granted, so I am only able to use the number of recruiters by state and quarter until
halfway through 2004, which I have included this extra control variable
for that portion of the sample. I also have detailed mortality data, through 2004. It is conceivable that deaths unrelated to
...
as well. Table \ref{Flo:Poisson Rec&Mort} shows these results. The
...
when I add the extra controls. %The estimates for contracts are only
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}
\includegraphics[scale=0.6]{C:/Users/garret/Documents/Research/Military/Output/graph_deathsvsrecruits_basic}
\caption{Graph of Monthly Recruits and Monthly Iraq/Afghanistan Combat Deaths}
\label{Flo:Monthly Deaths vs. Monthly Recruits}
\end{figure}

\begin{figure}
\includegraphics[scale=0.6]{C:/Users/garret/Documents/Research/Military/Output/PDF/histall4}
\caption{Active Duty/Total Deaths (2001-2010) and Active Duty Applicants/Contracts (1990-2006)}
\label{Flo:HISTO total deaths total recruits}
\end{figure}

\pagebreak{}
\begin{figure}
\includegraphics[scale=0.6]{C:/Users/garret/Documents/Research/Military/Output/PDF/hist_binomial}
\caption{Histogram of P-Values Testing Whether State Deaths All Came from the
...
\begin{table}
\caption{}
\label{Flo:Simple Deaths}\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/DeathsAnnualSimple}
\end{table}
...
\begin{table}
\caption{}
\label{Flo:Poisson Basic}\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.91]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefPbasic}
\end{table}
...
\begin{table}
\caption{}
\label{Flo:Poisson Rec&Mort}\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefPrec}
\end{table}
...
\begin{table}
\caption{}
\label{Flo:Media}\includegraphics[bb=45bp 0bp 612bp 730bp,clip, scale=0.91]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefcontigP}
\end{table}
...
\begin{table}
\caption{}
\label{Flo:Cumulative Lags}\includegraphics[bb=45bp 0bp 612bp 730bp,clip, scale=0.91]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefrunninglagsP}
\end{table}
...
\begin{table}
\caption{}
\label{Flo:Poisson Interactions}\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefPinteractions}
\end{table}
...
\begin{table}
\caption{}
\label{Flo: Recs by Quality}\includegraphics[bb=45bp 0bp 612bp 730bp,clip, scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/highqualitybytypeP}
\end{table}
...
\includegraphics[bb=45bp 0bp 612bp 730bp, scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefPwar}
\end{table}
...
shows a simple regression analysis of the the national time series of monthly combat deaths and monthly total applicants from October 2001 through July 2006. Graphically,
spikes in deaths after the initial invasion of Iraq and the first
...
in the current and/or previous month and recruits. In terms of elasticities,
as shown in the table, a one percent increase in deaths is associated
with a 1.5 to 2 percent decrease in applicants and a 2.0 to 2.7 percent
reduction in contracts. (Standard non-logged OLS regressions show
...
Although I consider the Poisson regressions to be the most natural fit for count data with a large number of zeros that have no log, I have exhaustively tested other models and find generally similar results across all specifications. I present all these results here. Ordinary least squares results for recruit levels (as opposed to logs) are shown in Appendix Table \ref{Flo:County-basic}.
 All observations are weighted by county population. 
%(I have
...
%There are of course methods
%to deal with the first problem and other ways to calculate elasticities,
%however the second reason makes the interpretation of the 2.7 coefficient
%fairly odd. It's hard to know what sort of effect is implied by a
%$2.7*\infty\%$ decrease in recruits. 
...
In addition to testing every recruit, I also test the model by using only deaths from active duty soldiers, as opposed to deaths of all soldiers (active, reserve, and guard) as in all other specification. Results, shown in Table~\ref{Flo:Rdeaths} appear slightly stronger, in that deterrent effects of in-county deaths are estimated to be slightly larger than one percent.
...
\begin{table}
\caption{}
\label{Flo:TABLE: Simple Time Series}\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/deathsvrecruits}
\end{table}
\clearpage{}

%Graph of recruit pct by population percent by state
\begin{figure}
\includegraphics[scale=0.6]{C:/Users/garret/Documents/Research/Military/Output/graph_table1_rec_pop}
...
\begin{figure}
\includegraphics[scale=0.6]{C:/Users/garret/Documents/Research/Military/Output/graph_table1_death_pop}
...
\begin{table}
\caption{}
\label{Flo:County-basic}\includegraphics[bb=45bp 0bp 612bp 730bp,clip, scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefW}
\end{table}

%4 Linear with logged LHS
\pagebreak{}
\clearpage{}
\begin{table}
\caption{}
\label{Flo:Elasticities}
\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefWLN}
\end{table}
...
\caption{}
\label{Flo:sqrt}
\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/sqrt}
\end{table}
...
\caption{}
\label{Flo:negbinom}
\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/negbinom}
\end{table}
...
\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/allrecPbasic}
\end{table}
...
\caption{}
\label{Flo:Rdeaths}
\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefPbasicR}
\end{table}
...
\caption{}
\label{Flo:redefPF1}
\includegraphics[bb=45bp 0bp 612bp 730bp,clip,scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefPF1}
\end{table}

%Non-Combat Deaths, 1990-2006 and 2001-2006
\pagebreak{}
\clearpage{}
\begin{table}
\caption{}
\label{Flo:alldeaths90}
\includegraphics[bb=45bp 0bp 692bp 780bp,clip,scale=1.0]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefPbasic90}
\end{table}

%9 Break recruits out by service branch
\pagebreak{}
\clearpage{}
\begin{table}
\caption{}
\label{Flo:Recruits by Service}\includegraphics[bb=45bp 0bp 612bp 730bp,clip, scale=0.91]{C:/Users/garret/Documents/Research/Military/Output/PDF/highqualitybyserviceP}
\end{table}
...
\begin{table}
\caption{}
\label{Flo:Death by Same-Service Other-Service}\includegraphics[bb=45bp 0bp 612bp 730bp,scale=0.91]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefbyserviceP}
\end{table}
...
\begin{table}
\caption{}
\label{Flo: Death by Service}\includegraphics[bb=45bp 0bp 612bp 730bp, scale=0.95]{C:/Users/garret/Documents/Research/Military/Output/PDF/redefbyservicedeath}
\end{table}
